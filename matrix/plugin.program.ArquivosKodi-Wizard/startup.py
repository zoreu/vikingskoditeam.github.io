        ...
        if response:
            ...
            if repoversion:
                ...
                if repo_response:
                    ...
                    logging.log("[Auto Install Repo] Instalado com sucesso", level=xbmc.LOGINFO)
                else:
                    logging.log_notify("[COLOR {0}]Repo Erro de instalação[/COLOR]".format(CONFIG.COLOR1),
                                       "[COLOR {0}]URL inválido para zip![/COLOR]".format(CONFIG.COLOR2))
                    logging.log("[Auto Install Repo] Não foi possível criar um URL funcional para o repositório. {0}".format(
            ...
            logging.log_notify("[COLOR {0}]Erro de instalação do Repo[/COLOR]".format(CONFIG.COLOR1),
                               "[COLOR {0}]Inválida addon.xml arquivo![/COLOR]".format(CONFIG.COLOR2))
            logging.log("[Auto Install Repo]Incapaz de ler o addon.xml arquivo.", level=xbmc.LOGERROR)
    elif not CONFIG.AUTOINSTALL == 'Yes':
        logging.log("[Auto Install Repo] Não habilitado", level=xbmc.LOGINFO)
    elif os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.REPOID)):
        logging.log("[Auto Install Repo] Repositório já instalado")
    ...
    if note_id:
        if note_id == CONFIG.NOTEID:
            if CONFIG.NOTEDISMISS == 'false':
                ...
                logging.log('[Notifications] Sem novas notificações.', level=xbmc.LOGINFO)
        ...
        logging.log('[Notifications] Arquivo de notificações {0} não formatado corretamente.'
...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        logging.log("[Build Installed Check] Build foi extraído {0}/100 with [ERROS: {1}]".format(CONFIG.EXTRACT,
                                                                                                    CONFIG.EXTERROR),
                    level=xbmc.LOGINFO)
        ...
        if yes:
            ...
            logging.log("[Build Installed Check] Nova instalação reset", level=xbmc.LOGINFO)
        else:
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]Parece que as configurações de skin não foram aplicadas à build.".format(CONFIG.COLOR2)
                ...
                yes = dialog.yesno(CONFIG.ADDONTITLE,
                                       ...
                                       nolabel='[B]Não, Cancelar[/B]', yeslabel='[B]Aplicar Fix[/B]')
                if yes:
                    xbmc.executebuiltin("PlayMedia(plugin://{0}/?mode=install&name={1}&url=gui)".format(CONFIG.ADDON_ID,
                                                                                                        quote_plus(CONFIG.BUILDNAME)))
                    logging.log("[Build Installed Check] Guifix tentando instalar")
                else:
                    logging.log('[Build Installed Check] Guifix url funcionando, mas cancelado: {0}'.format(gui_xml),
                                level=xbmc.LOGINFO)
    else:
        logging.log('[Build Installed Check] A instalação parece ter sido concluída corretamente', level=xbmc.LOGINFO)
...
def auto_clean():
    ...
    if next_cleanup <= tools.get_date() or freq == 0:
        ...
        logging.log("[Auto-limpeza] Next Clean Up {0}".format(CONFIG.NEXTCLEANDATE),
                    level=xbmc.LOGINFO)
    if service:
        if CONFIG.AUTOCACHE == 'true':
            logging.log('[Auto-limpeza] Cache: On', level=xbmc.LOGINFO)
            clear.clear_cache(True)
        else:
            logging.log('[Auto-limpeza] Cache: Off', level=xbmc.LOGINFO)
        if CONFIG.AUTOTHUMBS == 'true':
            logging.log('[Auto-limpeza] Old Thumbs: On', level=xbmc.LOGINFO)
            clear.old_thumbs()
        else:
            logging.log('[Auto-limpeza] Old Thumbs: Off', level=xbmc.LOGINFO)
        if CONFIG.AUTOPACKAGES == 'true':
            logging.log('[Auto-limpeza] Packages: On', level=xbmc.LOGINFO)
            clear.clear_packages_startup()
        else:
            logging.log('[Auto-limpeza] Packages: Off', level=xbmc.LOGINFO)
...
if CONFIG.get_setting('buildname'):
    ...
    if current_time >= epoch_check:
        logging.log("[Build Update Check] Iniciado", level=xbmc.LOGINFO)
        build_update_check()
else:
    logging.log("[Build Update Check] Próxima verificação: {0}".format(buildcheck), level=xbmc.LOGINFO)

# AUTO INSTALL REPO
if CONFIG.AUTOINSTALL == 'Yes':
    logging.log("[Auto Install Repo] Iniciado", level=xbmc.LOGINFO)
    auto_install_repo()
else:
    logging.log("[Auto Install Repo] Não Ativado", level=xbmc.LOGINFO)

# REINSTALL ELIGIBLE BINARIES
binarytxt = os.path.join(CONFIG.USERDATA, 'build_binaries.txt')
if os.path.exists(binarytxt):
    logging.log("[Binary Detection] Reinstalando addons Binários Elegíveis", level=xbmc.LOGINFO)
    from resources.libs import restore
    restore.restore('binaries')
else:
    logging.log("[Binary Detection] Addons binários qualificados para reinstalar", level=xbmc.LOGINFO)
    
# AUTO UPDATE WIZARD
if CONFIG.AUTOUPDATE == 'Yes':
    logging.log("[Auto Update Wizard] Iniciado", level=xbmc.LOGINFO)
...
if CONFIG.get_setting('installed') == 'true':
    logging.log("[Build Installed Check] Iniciado", level=xbmc.LOGINFO)
...
if CONFIG.get_setting('autoclean') == 'true':
    logging.log("[Auto-limpeza] Iniciado", level=xbmc.LOGINFO)
    auto_clean()
else:
    logging.log('[Auto-limpeza] Not Enabled', level=xbmc.LOGINFO)
