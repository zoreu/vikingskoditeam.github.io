...
url_b64_title = 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0FudWJpcy10ZWFtL0FuZHJvRmxpeC9tYWluL0NhdGVnb3JpYXM='
...
## MENU CONFIGURAÇÕES
menu_configuracoes = '[B][COLOR white]CONFIGURAÇÕES Andrômeda Flix[/COLOR][/B]'
...
## FAVORITOS
menu_favoritos = '[B][COLOR white]FAVORITOS Andrômeda Flix[/COLOR][/B]'
...
#### MENU VIP ################################################################
titulo_vip = '[COLOR white][B]ACESSO ANDRÔMEDA FLIX[/B][/COLOR] [COLOR gold][B](VIP)[/B][/COLOR]'
thumbnail_vip = 'https://i.imgur.com/5rgqF8K.png'
fanart_vip = 'https://i.imgur.com/nTIPRcu.png'
#### DESCRIÇÃO VIP ###########################################################
vip_descricao = 'Entre em contato para saber mais sobre a area VIP'
#### DIALOGO VIP - SERVIDOR DESATIVADO - CLICK ###################################
vip_dialogo = 'Vip Andrômeda Flix - Use nosso App oficial'
##SERIVODR VIP
url_server_vip = ''

thumb_server01 = "https://raw.githubusercontent.com/kodishmediacenter/Rework-hidra/main/icones/seriea.png"

## MULTLINK
## nome para $nome, padrão: lsname para $lsname
playlist_command = 'texto'
dialog_playlist = 'Andrômeda Flix - Selecione um item'
...
url_origem = 'https://pastebin.com/raw/kfwPCYmH'
url_origem = 'https://pastebin.com/raw/kfwPCYmH'
...
torrent_cli = xbmcaddon.Addon().getSetting("torrent")
lista = os.path.join(xbmcvfs.translatePath("special://home/addons/plugin.video.hidratuxedo/lista.xml"))
...
def sobre():
    xbmcgui.Dialog().ok('[B][COLOR white]AVISO[/COLOR][/B]','Rodando pelo Kodi 19. Necessario Elementum e Youtube Configurados Corretamente')
                ...
                try:
                    if linkedUrl=='':
                        ...
                        if adult == 'false' and re.search("ADULTOS",name,re.IGNORECASE) and name.find('(+18)') >=0:
                            ...
                            thumbnail2 = ''+thumbnail+'|User-Agent=Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0'
                            addDir(name.encode('utf-8', 'ignore'),linkedUrl,1,thumbnail2,fanArt,desc,genre,date,credits,year,director,writer,duration,premiered,studio,rate,originaltitle,country,rating,userrating,votes,aired)
                except:
...
def resolver(link,regex):
    ...
    try:
        if not link_decoded.startswith("plugin://plugin") and link_decoded.startswith('https://drive.google.com') == True:
            ...
            torrent_clix = int(torrent_cli)
            
            if torrent_clix == 0:
                resolved = 'plugin://plugin.video.elementum/play?uri='+link_decoded
                
            if torrent_clix == 1:
                resolved = 'plugin://plugin.video.torrest/play_magnet?magnet='+link_decoded
            ...
            if torrent_clix == 1:
                resolved = 'plugin://plugin.video.torrest/play_url?url='+link_decoded
...
def SKindex():
    #addDir('[COLOR white][B]Adicionar Lista M3U / XML [/B][/COLOR][COLOR cyan][B] ||| Hidra [/B][/COLOR][COLOR pink][B]Tuxedo |||[/B][/COLOR]','',201,thumb_server01,'','Servidores de Musica Online','','','','','','','','','','','','','','','','')
    #addDir('[COLOR white][B]Lista M3U / XML [/B][/COLOR][COLOR cyan][B] ||| Hidra [/B][/COLOR][COLOR pink][B]Tuxedo |||[/B][/COLOR]','',202,thumb_server01,'','Servidores de Musica Online','','','','','','','','','','','','','','','','')
    if favoritos == 'true':
        addDir(menu_favoritos,'',15,thumb_favoritos,'',desc_favoritos,'','','','','','','','','','','','','','','','')
    getData(url_principal, '')
    #addDir('[COLOR white][B]Servidores de Musica[/B][/COLOR][COLOR cyan][B] ||| Hidra [/B][/COLOR][COLOR pink][B]Tuxedo |||[/B][/COLOR]','',200,thumb_server01,'','Servidores de Musica Online','','','','','','','','','','','','','','','','')
    xbmcplugin.endOfDirectory(addon_handle)
