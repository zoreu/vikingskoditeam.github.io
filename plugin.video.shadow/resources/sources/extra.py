...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    for itt in search_url:
      for page in range(1,4):
        ...
        x,cook=cloudflare_request('https://extratorrent.cm/search/%s/?search=%s&x=0&y=0'%(str(page),itt))
        logging.warning('https://extratorrent.cm/search/%s/?search=%s&x=0&y=0'%(str(page),itt))
        regex='<tr class(.+?)</tr>'
