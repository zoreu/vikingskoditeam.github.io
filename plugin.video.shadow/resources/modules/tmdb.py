...
def adv_gen_window(url):
    ...
    class adv_gen_window(pyxbmct.AddonDialogWindow):
        def __init__(self,type):
            super(adv_gen_window, self).__init__('choose')
            wd=int(1250)
            hd=int(700)
            px=int(10)
            py=int(10)
        ...
        def radio_update(self,radiobutton,id):
            ...
            logging.warning('Clicked:'+str(id))
        ...
        def set_info_controls(self):
            # Edit
            edit_label = pyxbmct.Label('From year')
            ...
            # Edit
            edit_label = pyxbmct.Label('To year')
            ...
            #genere
            edit_label = pyxbmct.Label('Genre')
           ...
           for data in html['results']:
             ...
             if 'original_title' in data:
               ...
               if data['original_language']!='en':
                    ...
                    logging.warning('second original')
             ...
             trailer = "plugin://plugin.video.shadow?mode=25&id=%s&url=%s" % (id,tv_movie)
            ...
            xbmc.executebuiltin((u'Notification(%s,%s)' % ('Shadow', 'No Trailer...Line:'+str(lineno)+' E:'+str(e))).encode('utf-8'))
...
def get_movies(url,local=False,reco=0,global_s=False):
   ...
   if '/search' in url and 'page=1' in url:
       ...
       if search_entered+'$$$'+tv_movie not in all_s_strings:
         ...
         dbcur.execute("INSERT INTO search_string2 Values ('%s','%s')"%(urllib.unquote_plus(search_entered.replace("'","%27")),tv_movie))
         dbcon.commit()
   html={}
   html['results']=[]
   regex='page=(.+?)$'
   ...
   for  name,url,mode,icon,fan,plot,year,original_name,id,rating,new_name,year,isr,genere,trailer,watched,fav_status,xxx,max_page,all_res in all_in_data:
            if local:
                ...
                aa=addDir3(name,url,mode,icon,fan,plot,data=year,original_title=original_name,id=id,rating=rating,heb_name=new_name,show_original_year=year,generes=genere,trailer=trailer,watched=watched,fav_status=fav_status,collect_all=True,all_w=all_w)
                all_d.append(aa)
   ...
   if   max_page==-1:
        if not global_s:
            xbmcgui.Dialog().ok('Shadow','[COLOR aqua][I] No results[/I][/COLOR]')
   ...
   if max_page>int(match[0]):
        if local:
            ...
            mode=14
        aa=addDir3(('[COLOR aqua][I]Page %s Out of %s (%s Results)[/I][/COLOR]'%(str(int(match[0])+1),str(max_page),str(all_res))).decode('utf8'),link+'page='+str(int(match[0])+1),mode,BASE_LOGO+'next.png','https://cdn4.iconfinder.com/data/icons/arrows-1-6/48/1-512.png','Results'.decode('utf8'),show_original_year='999',data='999',collect_all=True)
        all_d.append(aa)
...
def get_seasons(name,url,iconimage,fanart,description,data,original_title,id):
   all_d=[]
   payload= {
   ...
   for data in html['seasons']:
     ...
     if 'season 0' in new_name:
        continue
     aa=addDir3( '[COLOR %s]'%color+new_name+'[/COLOR]',url,19,icon,fan,datea+plot,data=year,original_title=original_name,id=id,season=season,tmdbid=tmdbid,show_original_year=show_original_year,heb_name=original_name,ep_number=ep_number,watched_ep=watched,remain=remain,premired=premired)
     all_d.append(aa)
   xbmcplugin .addDirectoryItems(int(sys.argv[1]),all_d,len(all_d))
...
def get_episode(name,url,iconimage,fanart,description,data,original_title,id,season,tmdbid,show_original_year):
   import _strptime
   all_d=[]
   ...
   for data in html['episodes']:
     ...
     if  Addon.getSetting("disapear")=='true' and color=='red':
       ...
       aa=addDir3( '[COLOR %s]'%color+new_name+'[/COLOR]', url,15, icon,image,datea+plot,data=year,original_title=original_name,id=id,season=season,episode=data['episode_number'],eng_name=eng_name,show_original_year=show_original_year,heb_name=original_name,watched=watched,fav_status=fav_status,all_w=all_w,premired=premired)
       all_d.append(aa)
   #dbcur.close()
   #dbcon.close()
   xbmcplugin .addDirectoryItems(int(sys.argv[1]),all_d,len(all_d))
