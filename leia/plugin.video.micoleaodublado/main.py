...
##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = "[B][COLOR orange]■[/COLOR] BEM VINDO AO [COLOR orange]MICO-LEÃO DUBLADO[/COLOR][/B]"
###  DESCRIÇÃO DO ADDON ###############################################################
title_descricao = "O melhor conteúdo em filmes e séries dubladas, um addon feito para toda a família!"
####  LINK DO TITULO DE MENU  #########################################################
url_title = ''

#### MENSAGEM BEM VINDOS  #############################################################
titulo_boas_vindas = "BEM-VINDOS"
mensagem_bem_vindo = "[B][COLOR white]Bem vindo ao [COLOR orange]MICO-LEÃO DUBLADO[/COLOR] [/COLOR]\n[COLOR white]Curta nossa página no facebook:\n[/COLOR][COLOR blue]https://fb.com/micoleaodublado [/COLOR][/B]"
####  MENSAGEM SECUNDARIA ######################################################
mensagem_secundaria = "[COLOR white][B]CONTEÚDOS NOVOS TODA SEMANA![/B][/COLOR]"
####  TEMPO DA MENSAGEM EM MILISEGUNDOS ###############################################
time_msg = 15000


## MENU CONFIGURAÇÕES
menu_configuracoes = "[B][COLOR orange]■[/COLOR] CONFIGURAÇÕES [COLOR orange]ADDON E ELEMENTUM[/COLOR][/B]"
thumb_icon_config = 'https://i.ibb.co/qnhDt5d/CAPA-CONFIGURA-ES.jpg'
desc_configuracoes = "Entre aqui para acessar as configurações do Mico-leao dublado e para ativar o download do plugin [COLOR orange]Elementum[/COLOR], necessário para usufruir dos consteúdos disponiveis."
...
deletar_favoritos = base64.b64decode(favoritos_content).decode('utf-8')
...
def SetView(name):
    ...
    if name == 'Fanart':
        try:
            ...
            pass
iconBoasVindas = 'https://i.ibb.co/jhytX0h/CAPA-BEMVINDO.jpg'
def SKindex():
    #addDir(name,url,mode,iconimage,fanart,description)
    if addon.getSetting('mensagem1') == 'true':
        xbmcgui.Dialog().ok(titulo_boas_vindas,mensagem_bem_vindo)
    addDir(title_menu,url_title,1,iconBoasVindas,'',title_descricao)
    ### VIP ##############
    
    getData(deletar_favoritos, '')   
    ...
    if addon.getSetting('mensagem2') == 'true':
        ...
        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,getRequest2('https://pastebin.com/raw/mAPS20VG', 'https://pastebin.com/raw/mAPS20VG'), time_msg, __icon__))
