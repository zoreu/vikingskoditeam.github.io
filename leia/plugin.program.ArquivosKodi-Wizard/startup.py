	...
	if len(match) > 0:
		...
		if version > BUILDVERSION:
			if DISABLEUPDATE == 'false':
				wiz.log("[Verificar atualizações] [Versão instalada: %s] [Versão atual: %s] Abrindo janela de atualização" % (BUILDVERSION, version), xbmc.LOGNOTICE)
		...
		else: wiz.log("[Verificar atualizações] [Versão instalada: %s] [Versão atual: %s]" % (BUILDVERSION, version), xbmc.LOGNOTICE)
	else: wiz.log("[Verificar atualizações] ERRO: Não é possível encontrar a versão da build no arquivo de texto de build", xbmc.LOGERROR)
...
def checkSkin():
	...
	if not DEFAULTSKIN == '':
		if os.path.exists(os.path.join(ADDONS, DEFAULTSKIN)):
			if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Parece que a skin foi adiada para [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, SKIN[5:].title()), "Você gostaria de colocar a skin de volta para:[/COLOR]", '[COLOR %s]%s[/COLOR]' % (COLOR1, DEFAULTNAME)):
		...
		if len(skinlist) > 0:
			if len(skinlist) > 1:
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems that the skin has been set back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, SKIN[5:].title()), "Would you like to set the skin back to:[/COLOR]", '[COLOR %s]%s[/COLOR]' % (COLOR1, skinname[0])):
					gotoskin = skinlist[0]
					gotoname = skinname[0]
				else: wiz.log("A skin não foi reiniciada", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true')
		else: wiz.log("Não foram encontradas peles na pasta addons.", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true'); gotoskin = False
	if gotoskin:
		...
		else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Troca de skin cronometrada![/COLOR]' % COLOR2)
	wiz.log("[Build Check] Extremidade de verificação de skin inválida", xbmc.LOGNOTICE)
...
if not ADDONID == path[1]: DIALOG.ok(ADDONTITLE, '[COLOR %s]Certifique-se de que a pasta de plugin seja a mesma que a ADDON_ID.[/COLOR]' % COLOR2, '[COLOR %s]Plugin ID:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, ADDONID), '[COLOR %s]Plugin Folder:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, path)); wiz.log("[Path Check] ADDON_ID e pasta plugin não corresponde. %s / %s " % (ADDONID, path))
...
if KODIADDONS in ADDONPATH:
	...
	if os.path.exists(newpath):
		wiz.log("Pasta já existe, casa de limpeza", xbmc.LOGNOTICE)
...
wiz.log("[Repo Auto Instalar] Começou", xbmc.LOGNOTICE)
	...
	if workingxml == True:
		...
		if len(ver) > 0:
			...
			if workingrepo == True:
				...
				xbmc.sleep(500)
				wiz.forceUpdate(True)
				wiz.log("[Repo Auto Instalar] Instalado com sucesso", xbmc.LOGNOTICE)
			else: 
				wiz.LogNotify("[COLOR %s]Erro de instalação do Repo[/COLOR]" % COLOR1, "[COLOR %s]Invalid url for zip![/COLOR]" % COLOR2)
				wiz.log("[Repo Auto Instalar] Não foi possível criar uma url de trabalho para repositório. %s" % workingrepo, xbmc.LOGERROR)
		else:
			wiz.log("URL inválido para Repo Zip", xbmc.LOGERROR)
	else: 
		wiz.LogNotify("[COLOR %s]Erro de instalação do Repo[/COLOR]" % COLOR1, "[COLOR %s]Invalid addon.xml file![/COLOR]" % COLOR2)
		wiz.log("[Repo Auto Instalar] Não é possível ler o arquivo addon.xml.", xbmc.LOGERROR)
elif not AUTOINSTALL == 'Yes': wiz.log("[Repo Auto Instalar] Não habilitado", xbmc.LOGNOTICE)
elif os.path.exists(os.path.join(ADDONS, REPOID)): wiz.log("[Repo Auto Instalar] Repositório já instalado")

wiz.log("[Wizard Auto Instalar] Começou", xbmc.LOGNOTICE)
if AUTOUPDATE == 'Yes':
	wiz.wizardUpdate('startup')
else: wiz.log("[Wizard Auto Instalar] Not Enabled", xbmc.LOGNOTICE)

wiz.log("[Notifications] Começou", xbmc.LOGNOTICE)
if ENABLE == 'Yes':
	if not NOTIFY == 'true':
			...
			if not id == False:
				try:
					...
					if id == NOTEID:
						...
						else: wiz.log("[Notifications] id[%s] Demitido" % int(id), xbmc.LOGNOTICE)
					elif id > NOTEID:
					...
					wiz.log("Erro janela de notificações: %s" % str(e), xbmc.LOGERROR)
			else: wiz.log("[Notifications] Arquivo de texto não formatado corretamente")
		else: wiz.log("[Notifications] URL(%s): %s" % (NOTIFICATION, url), xbmc.LOGNOTICE)
	else: wiz.log("[Notifications] Desligado", xbmc.LOGNOTICE)
else: wiz.log("[Notifications] Não habilitado", xbmc.LOGNOTICE)

wiz.log("[Installed Check] Começou", xbmc.LOGNOTICE)
if INSTALLED == 'true':
	if KODIV >= 17:
		wiz.kodi17Fix()
		...
		wiz.log("[Installed Check] A Build foi extraída %s/100 com [ERROS: %s]" % (EXTRACT, EXTERROR), xbmc.LOGNOTICE)
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s]%s[/COLOR] [COLOR %s]não foi instalado corretamente!' % (COLOR1, COLOR2, BUILDNAME), 'Installed: [COLOR %s]%s[/COLOR] / Erro Contar: [COLOR %s]%s[/COLOR]' % (COLOR1, EXTRACT, COLOR1, EXTERROR), 'Você gostaria de tentar novamente?[/COLOR]', nolabel='[B]Não, obrigado![/B]', yeslabel='[B]Sim, tentar[/B]')
		wiz.clearS('build')
		FAILED = True
		if yes: 
		...
		else: wiz.log("[Installed Check] Reinstalação Ignorada")
		...
		if not wiz.currSkin() == defaults and not BUILDNAME == "":
			...
			if gui == 'http://':
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Parece que as configurações da skin não foram aplicadas ao build." % COLOR2, "Infelizmente nenhuma correção gui foi attatched para a build", "Você precisará reinstalar a build e certifique-se de fazer uma força fechar[/COLOR]")
			elif wiz.workingURL(gui):
				yes=DIALOG.yesno(ADDONTITLE, '%s não foi instalado corretamente!' % BUILDNAME, 'Parece que as configurações da skin não foram aplicadas à build.', 'Você gostaria de aplicar o GuiFix?', nolabel='[B]Não, cancele.[/B]', yeslabel='[B]Aplicar Fix[/B]')
				if yes: wiz.ebi("PlayMedia(plugin://%s/?mode=install&name=%s&url=gui)" % (ADDON_ID, urllib.quote_plus(BUILDNAME))); wiz.log("[Installed Check] Guifix tentando instalar")
				else: wiz.log('[Installed Check] Guifix url trabalhando, mas cancelado: %s' % gui, xbmc.LOGNOTICE)
			else:
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Parece que as configurações da skin não foram aplicadas ao build." % COLOR2, "Infelizmente nenhuma correção gui foi attatched para o build", "Você precisará reinstalar a build e certifique-se de fazer uma força fechar[/COLOR]")
				wiz.log('[Installed Check] Guifix url not working: %s' % gui, xbmc.LOGNOTICE)
	else:
		wiz.log('[Installed Check] A instalação parece estar concluída corretamente', xbmc.LOGNOTICE)
...
if FAILED == False:
	...
	if not WORKING:
		...
		if SKIN in ['skin.confluence', 'skin.estuary'] and not DEFAULTIGNORE == 'true':
			checkSkin()
			wiz.log("[Build Check] Build Installed: Checking Atualizações", xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			checkUpdate()
			...
			wiz.log("[Build Check] Build Installed: O próximo check não é até: %s / TODAY is: %s" % (BUILDCHECK, str(TODAY)), xbmc.LOGNOTICE)
...
if int(total_sizetext) > filesize:
	...
	wiz.log("[Limpador automático] Limpador de pacotes acionado", xbmc.LOGNOTICE)
...
if int(total_sizetext2) > filesize_thumb:
	...
	wiz.log("[Limpador automático] Limpador de polegares acionado", xbmc.LOGNOTICE)
