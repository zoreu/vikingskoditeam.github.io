...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font10', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font10',BorderWidth=10):
			...
			header = '[COLOR %s]Advanced Settings Configurador[/COLOR]' % (COLOR2)
			...
			header = '[COLOR %s]Video Cache tamanho[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s]Quando definido para [COLOR %s]0[/COLOR] o cache será gravado em disco em vez de RAM.  Nota: Definir isso muito alto pode fazer com que o Kodi trave se não conseguir RAM suficiente (1/3 de memória livre: [COLOR %s]%s[/COLOR])[/COLOR]" % (COLOR2, COLOR1, COLOR1, freeMemory)
			...
			current1 = '[COLOR %s]Atual:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.currentVideo)
			...
			header = '[COLOR %s]Tempo limite de curl/CURL Baixa Velocidade[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s][B]curlclienttimeout[/B] -  é o tempo em segundos para quanto tempo leva para a conexão libcurl vai tempo limite e [B]curllowspeedtime[/B] -  é o tempo em segundos para libcurl considerar uma conexão de baixa velocidade.  Para conexões mais lentas, defina-o para 20.[/COLOR]" % COLOR2
			...
			current2 = '[COLOR %s]Atual:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, self.currentCurl)
			recommended2 = '[COLOR %s]Recomendado:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, self.recommendedCurl)
			...
			header = '[COLOR %s]Ler fator buffer[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s]Se o Kodi estiver carregando um arquivo bruto bluray típico a 36 Mbit/s, então um valor de 2 precisará de pelo menos 72 Mbit/s de largura de banda de rede. No entanto, ao contrário da configuração RAM, você pode aumentar com segurança esse valor por mais alto que quiser, e o Kodi não vai travar.[/COLOR]" % COLOR2
			...
			current3 = '[COLOR %s]Atual:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, self.currentRead)
			recommended3 = '[COLOR %s]Recomendado:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, self.recommendedRead)
			...
			self.addControl(self.Header4)
			msg4 = "[COLOR %s]Essa configuração forçará o Kodi a usar um cache para todos os arquivos de vídeo, incluindo rede local, internet e até mesmo o disco rígido local. O valor padrão é 0 e só armazenará vídeos que usam caminhos/fontes de arquivos da Internet.[/COLOR]" % COLOR2
			...
			self.Button0 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B1, (W/2)-(BorderWidth*4), 30, '0: Armazenar em buffer de arquivos da Internet', font='font10', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button1 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B2, (W/2)-(BorderWidth*4), 30, '1: Armazenar todos os sistemas de arquivos', font='font10', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button2 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B3, (W/2)-(BorderWidth*4), 30, '2: Apenas sistemas de arquivos verdadeiros', font='font10', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button3 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B4, (W/2)-(BorderWidth*4), 30, '3: Sem Buffer', font='font10', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			...
			self.buttonWrite=xbmcgui.ControlButton(leftside,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Gravar arquivo",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
		...
		def updateCurrent(self, control):
			if control == self.videoCacheSize:
				...
				current = '[COLOR %s]Atual:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, int(self.currentVideo))
				...
				current = '[COLOR %s]Atual:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, int(self.currentCurl))
				...
				current = '[COLOR %s]Atual:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, int(self.currentRead))
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Atualmente, há um ativo [COLOR %s]AdvancedSettings.xml[/COLOR], Gostaria de removê-lo e continuar?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Remover configurações[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar Write[/COLOR][/B]")
				if choice == 0: return
			...
			if KODIV < 17:
				...
				with open(ADVANCED, 'w+') as f:
					...
					f.write('</advancedsettings>\n')
				f.close()
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Arquivo Gravado[/COLOR]' % COLOR2)
			self.CloseWindow()
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Atualização avaliável para build instalada:\n[COLOR %s]%s[/COLOR]\n\nCurrent Versão: v[COLOR %s]%s[/COLOR]\nLatest Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendado: Instalação zerada[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
			self.msgcurrent = "Executando a versão mais recente da Build instalada:\n[COLOR %s]%s[/COLOR]\n\nCurrent Version: v[COLOR %s]%s[/COLOR]\nLatest Versão: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendado: Instalação zerada[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
		...
		def doFreshInstall(self):
			wiz.log("[Check Atualizações] [Versão instalada: %s] [Versão atual: %s] [Usuário selecionado: Zerar kodi e instalar build]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Check Atualizações] [Next Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
		...
		def doNormalInstall(self):
			wiz.log("[Check Atualizações] [Versão instalada: %s] [Versão atual: %s] [Usuário selecionado: Normal Instalar a Build]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Check Atualizações] [Next Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
		...
		def doIgnore(self):
			wiz.log("[Check Atualizações] [Versão instalada: %s] [Versão atual: %s] [User Selected: Ignorar 3 Dias]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Check Atualizações] [Next Check: %s]" % str(THREEDAYS), xbmc.LOGNOTICE)
