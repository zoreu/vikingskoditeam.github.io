...
def getConfig():
    ...
    if uh is False:
        print_('Não foi possível recuperar speedtest.net configuração: %s' % e)
        sys.exit(1)
    configxml = []
    while 1:
...
def speedtest(
    ...
    dp.update(0)
    build_user_agent()
    print_('Recuperando a configuração speedtest.net...')
    line2 = \
        '[COLOR %s]Recuperando a configuração speedtest.net...[/COLOR]' \
        % COLOR2
    dp.update(2, line1, line2)
    try:
        config = getConfig()
    except URLError:
        print_('Não é possível recuperar a configuração do teste de velocidade')
        sys.exit(1)

    print_('Recuperando speedtest.net lista de servidores...')
    line3 = '[COLOR %s]Recuperando speedtest.net lista de servidores...[/COLOR]' \
        % COLOR2
    ...
    if share:
        ...
        if f is False:
            print_('Não foi possível enviar resultados para speedtest.net: %s' % e)
            sys.exit(1)
        response = f.read()
        code = f.code
        f.close()
        ...
        if int(code) != 200:
            print_('Não foi possível enviar resultados para speedtest.net')
            sys.exit(1)
        ...
        print_('Compartilhar resultados: %s://www.speedtest.net/result/%s.png'
               % (scheme, resultid[0]))
